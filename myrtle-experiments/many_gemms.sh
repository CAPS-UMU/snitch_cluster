...
extractKernelTime="$here/extractDataFromJsons.py"
verifyScript="$here/verify.py"
...
runAndExtract(){
    ...
    for ts in $(grep -oE $uniquePointRegex $ss)
            ...
            then
                ...
                nohup bash run_and_extract_time.sh $buildDir $extractKernelTime $expName $logs $M $N $K $m $n $k $rootDir &> "$buildDir/output.txt" & 
                counter=$((counter+1))
            fi 
